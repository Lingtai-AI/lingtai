...
- **`upgrade.go`** — startup TUI binary upgrade flow: after install-method routing (`tui/main.go:113-119`), all supported installs update through the GitHub Release installer (`install.sh --update`). Homebrew installs get an explicit `[y/N]` (default No) prompt that states it converts the install from Homebrew-managed to GitHub-Release-managed, includes `brew reinstall lingtai-tui` rollback guidance, and — when other TUI windows are running — first puts affected agents to sleep and stops those processes before routing through the Homebrew adoption backend. Source/user-local installs get their own explicit `[y/N]` prompt routing through the source updater backend. Both share the same `install.sh --update` contract; neither runs `brew upgrade`. Unknown installs are not mutated at startup (version-only).
...
- **`tui_process_unix.go` / `tui_process_windows.go`** — platform helpers for detecting and stopping other running `lingtai-tui` binaries before an in-app binary update (e.g. the Homebrew adoption flow).
...
- **`lingtai-tui self-update` subcommand** (`selfUpdateMain` in `tui/main.go`): runs `config.RunManualTUIUpdate` through the detected install-method backend. Both source/user-local and Homebrew installs update through the GitHub Release installer (`install.sh --update`) — Homebrew installs are adopted onto GitHub-Release management (rollback: `brew reinstall lingtai-tui`); unknown installs produce non-mutating GitHub Release / curl-installer guidance without trying brew.
...
- **TUI binary upgrade:** `tui/main.go:111-120` checks for a newer release, detects the current install method (`config.DetectCurrentTUIInstall`), then routes on `install.Method` and delegates newer-release handling to `upgrade.go` (`handleTUIUpgrade`) for Homebrew and source/user-local installs. Both require an explicit `[y/N]` (default No) confirmation before running the GitHub Release installer (`install.sh --update`); neither runs `brew upgrade`. The Homebrew prompt states it converts the install to GitHub-Release-managed (rollback `brew reinstall lingtai-tui`) and, when other TUI windows run, first puts their agents to sleep and stops those processes. Unknown installs keep the version-only startup path and do not mutate.
