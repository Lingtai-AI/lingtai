...
- **`tui/main.go:844-918`** — `cleanMain`: suspend agents in `.lingtai/` (10s timeout), then `os.RemoveAll`.
- **`tui/main.go:920-968`** — `postmanMain`: parse `--port`, collect watch directories, call `postman.Run`.
