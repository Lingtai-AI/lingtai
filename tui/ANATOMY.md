...
- **`tui/main.go:33-1332`** — single-file `package main`. The version stamp (`tui/main.go:31`, set via `-ldflags`), welcome/help text, Rust toolchain startup guidance, and interactive entry (`tui/main.go:33-397`). After parsing subcommands, `main()` runs the no-project decision gate (below), then global migrations, checks invariants (init.json all-or-nothing, exactly-one-orchestrator), handles upgrade prompts and first-run wizard routing, then launches Bubble Tea.
- **No-project decision gate (`tui/main.go:101-149`)** — before ANY of `config.GlobalDir()`, `globalmigrate.Run`, `MigrateLegacyLanguage`/`LoadTUIConfig`'s writes, `ValidateCodexAuthOnStartup`, `showWelcome`, `maybeShowAgentCount`, `process.InitProject`, `config.Register`, `preset.PopulateBundledLibrary`, or `EnsureRuntime`/`preset.Bootstrap`, `main()` calls the pure `tui.ProbeNoProjectPure(projectDir)` (an `os.Lstat` check, never `Stat`) at `tui/main.go:126`, failing closed (`os.Exit(1)`) on any non-nil `probeErr` before it. When it reports no project, `runNoProjectLauncher` (`tui/main.go:411-438`) runs `tui.LauncherRootModel` (`tui/internal/tui/launcher.go`) in its own `tea.Program`, separate from the real `App`'s. The launcher's `LauncherResult.Kind` then decides: `DecisionCancel` exits with zero writes; `DecisionOpenExisting` reassigns `projectDir` to the chosen root and falls through into the SAME pipeline below (migrations, invariants, `InitProject`-as-noop, register, bootstrap — exactly as if the user had `cd`'d there); `DecisionCreate` calls `runCreatedProject` (`tui/main.go:440-478`), which resolves `i18n.SetLang` BEFORE printing any post-commit warning (so the warning renders in the user's configured locale, not the i18n default) and reports any post-commit warnings from the finalizer (`tui/internal/tui/project_create.go`'s `RunProjectCreate`, already run inside the launcher's program), then constructs `App` normally for the newly published project. CLI subcommands (`list`, `clean`, `doctor`, `spawn`, ...) never reference the gate — verified by `TestNoProjectGate_SubcommandsNeverCallLauncher` (`tui/main_no_project_gate_test.go`). Gate ordering itself is proven by `TestNoProjectGate_PrecedesGlobalDirAndMigrations` in the same file.
- **No-project launcher Open Existing (`tui/internal/tui/launcher.go:96-461`)** — the pre-App launcher embeds the established registry-mode `ProjectsModel` (`tui/internal/tui/projects.go:36-459`) instead of maintaining a second project catalog. Its validated `ProjectsAgentSelectedMsg.Record.Project` becomes `DecisionOpenExisting`; the child back message returns to launcher Choose. Create remains draft-only until the existing staging/rename commit path.
...
- **`upgrade.go`** — startup TUI binary upgrade flow: after install-method routing (`tui/main.go:161-170`), Homebrew installs keep the existing prompt that detects other running TUI windows, puts affected agents to sleep, stops old TUI processes, and runs `brew upgrade` before asking the user to relaunch; source/user-local installs get a separate explicit `[y/N]` prompt that routes through the source updater backend (`install.sh --update`). Unknown installs are not mutated at startup (version-only).
...
- **`tui/internal/headless/`** — JSON-emitting non-interactive surface. `RunPresets` (lists templates/saved presets as JSON), `RunSpawn` (creates a project + launches an agent), and `ExitError` (structured error codes). Wired from `main.go` via `bootstrapMain` (`tui/main.go:1164`), `presetsMain` (`tui/main.go:1252`), and `spawnMain` (`tui/main.go:1276`). For agents and scripts that drive `lingtai-tui` without the Bubble Tea UI.
...
- **TUI binary upgrade:** `tui/main.go:151-175` checks for a newer release, detects the current install method (`config.DetectCurrentTUIInstall`), then routes on `install.Method` (`tui/main.go:161-170`) and delegates newer-release handling to `upgrade.go` (`handleTUIUpgrade`) for Homebrew and source/user-local installs. Homebrew keeps the existing confirmation flow, can put agents in their projects to sleep, stops other TUI processes, runs the Homebrew backend, and asks the user to relaunch. Source/user-local installs require an explicit `[y/N]` yes before running the source updater backend through `install.sh --update`; unknown installs keep the version-only startup path and do not mutate.
...
- **No-project launcher is a separate `tea.Program`, not a fake `App`.** `App` (`tui/internal/tui/app.go`) assumes a resolved `projectDir`/orchestrator context throughout — `NewApp`'s branching, the visit machinery, and `newMailForCurrentContext` all read fields that only make sense once a project exists. Rather than construct `App` with an empty/fake project path (which would scatter nil-path special cases across the ~20k-line `internal/tui` package), the no-project case runs an entirely separate root model (`tui.LauncherRootModel`, `tui/internal/tui/launcher.go`) in its own `tea.Program` from `main.go`, and only constructs the real `App` after a `LauncherResult` names a concrete project root (either picked via Open Existing or freshly committed via Create). See `tui/internal/tui/ANATOMY.md`'s "No-project launcher" section for the full component breakdown.
