...
- **Bootstrap sequence** (`main.go:228-273`): on every launch, the TUI runs `config.MigrateLegacyLanguage` → `config.NeedsVenv` (for setup banner) → `config.EnsureRuntime` (create/repair venv if needed, then always run the non-blocking `CheckUpgrade`) → `config.EnsureAddons` → `preset.Bootstrap` → `tui.ExportCommandsJSON` → `maybePromptRustToolchain` (one-time optional Rust/Cargo prompt only when file search is on Python fallback and no cargo is on PATH). `CheckUpgrade` auto-upgrades the `lingtai` runtime package and explicitly includes the curated LingTai MCP packages (`lingtai-imap`, `lingtai-telegram`, `lingtai-feishu`, `lingtai-wechat`, `lingtai-whatsapp`) so addon fixes are not stranded by pip's only-if-needed dependency strategy. It does not eager-upgrade arbitrary transitive dependencies. See `tui/internal/config/ANATOMY.md`.
...
- **Curated MCP packages upgrade with the runtime.** `config.CheckUpgrade` on every launch upgrades `lingtai` plus the curated LingTai MCP packages (`lingtai-imap`, `lingtai-telegram`, `lingtai-feishu`, `lingtai-wechat`, `lingtai-whatsapp`) explicitly, avoiding global eager dependency upgrades while still pulling addon releases that satisfy existing loose dependency bounds. Users never install MCP packages individually.
