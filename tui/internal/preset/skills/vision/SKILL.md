...
description: >
  ...
  if the LLM provider supports image input, (2) the `minimax-media` skill
...
└── NO  → does the user have a MiniMax coding-plan key?
         ...
         ├── YES → see `minimax-media` skill (use `understand_image` MCP tool)
...
## Path 2 — MiniMax via `minimax-media` Skill
...
Read the **`minimax-media`** skill — it covers credential sourcing, region selection, and points at the live docs for tool parameters. MCP server registration itself is owned by `lingtai-mcp`.
...
- You need to *generate* an image — use `minimax-media` (`text_to_image`).
