...
### `/notification` — show recent notification blocks
**Usage:** `/notification`
...
conversation rail between expanded and fully hidden. Tab can enter or leave the
rail only while it is expanded. At 84 columns or narrower, the rail is
automatically hidden. `/agents` remains the canonical all-width fallback and
selects from exactly the same rows.
