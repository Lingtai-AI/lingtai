...
- Then explain the architecture. **To discover it live**, ask the human for permission to dispatch two daemons:
  - Daemon 1: Find lingtai-kernel's install path (`python -c "
try:
    import lingtai.kernel as _kernel
    print(_kernel.__file__)
except ImportError:
    import lingtai_kernel
    print(lingtai_kernel.__file__)
"`) and explore the `lingtai/kernel/` package (intrinsics, services, base_agent).
