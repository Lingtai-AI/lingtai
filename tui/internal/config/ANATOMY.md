...
- **`tui/internal/config/venv.go:401-407`** — `CheckUpgrade`: thin wrapper over `UpgradePythonRuntime(globalDir, false, ...)` that returns `true` only when a real PyPI upgrade or dev-mode conversion was verified post-install. Used by `EnsureRuntime`, where any failure is silent so the TUI can still launch.
- **`tui/internal/config/venv.go:420-446`** — `EnsureRuntime` / `EnsureRuntimeQuiet`: startup helpers that ensure the managed Python runtime exists, then always run the non-blocking `CheckUpgrade` path after a successful ensure. This is what converts an existing importable PyPI runtime into editable/dev mode on local development machines.
...
- **`tui/internal/config/tui_updater.go`** — `TUIUpdater` interface plus `RunTUIUpdate` / `RunManualTUIUpdate` backend routing. GitHub Release is the only binary update source; no backend runs `brew update`/`brew upgrade`. The source backend (`sourceTUIUpdater.Upgrade`) requires a known release tag, reads source metadata from `install.json`, then runs the shared `runInstallShAdoption` helper: the versioned `install.sh --update --prefix <prefix> --version <tag> --non-interactive` contract, followed by verification of the new binary version, refreshed source metadata, and runtime venv health. The Homebrew backend (`homebrewTUIUpdater.Upgrade`) **adopts** the install onto the same GitHub-Release-managed path: it resolves a prefix via `homebrewAdoptionPrefix` (prefers `brew --prefix`, else infers the Homebrew prefix from the running executable's `Cellar`/known-prefix path), emits `brew reinstall lingtai-tui` rollback guidance, then runs `runInstallShAdoption` into `<prefix>/bin`. After adoption, `install.json` records source metadata for the active `<prefix>/bin/lingtai-tui`, so `detectTUIInstallMethod` reports source (not Homebrew) on the next launch. Unknown installs remain explicit non-mutating guidance pointing at the GitHub Release / curl installer. Callers keep version checks/prompting; the backend owns only the mutation-or-guidance for one install method.
- **`tui/internal/config/venv.go:1156-1287`** — `UpgradePythonRuntime`: checks the installed Python `lingtai` runtime. If a local dev checkout is discovered and the runtime is not already editable for that checkout, it runs editable install (`uv pip install -e <lingtai-kernel> -p <venv>` or venv `pip install -e <lingtai-kernel>`) before any PyPI comparison. Otherwise it preserves existing editable installs, compares to PyPI, runs upgrade if needed, verifies post-install import/version, and writes/refreshes the runtime environment marker best-effort after successful verified states.
...
- **Called from:** `tui/main.go`, the manual `lingtai-tui self-update` path, `tui/internal/tui/firstrun.go`, and `tui/internal/headless/spawn.go` — startup, manual self-update, first-run, and headless bootstrap paths.
- **Calls out:** PyPI API (`pypi.org/pypi/lingtai/json`), GitHub API (`api.github.com/repos/Lingtai-AI/lingtai/releases/latest`), `uv` / `pip` CLI, the GitHub Release `install.sh --update` installer for all supported TUI binary updates (including adopting Homebrew installs), and `brew --prefix` only to resolve the adoption target for a Homebrew install.
- **Bootstrap sequence:**
