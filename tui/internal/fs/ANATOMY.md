...
| `MailCache.RefreshRecent(limit)` | `tui/internal/fs/mail.go:318` | bounded refresh: directory listings only, newest `limit` ids selected before any `message.json` is read, retained entries reused, result trimmed to the newest `limit` by timestamp; `limit <= 0` delegates to `Refresh()`, which is how the unlimited opt-in reaches the disk |
| `mailboxIDsIn` / `readMailboxMessage` | `tui/internal/fs/mail.go:426`, `tui/internal/fs/mail.go:462` | the listing/body split that makes the bound possible: names are enumerated without opening a single body |
| `RecentMailboxIDs(folder, limit)` | `tui/internal/fs/mail.go:451` | the same window as a standalone primitive for readers outside this package (the `/mailbox` page): newest `limit` leaf ids, oldest-first, chosen from the directory listing alone; `limit <= 0` returns every id |
