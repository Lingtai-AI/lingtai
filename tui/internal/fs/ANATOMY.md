...
| `SumMoltSessionTokenLedger` | `tui/internal/fs/agent.go:645-659` | sums cached non-daemon token-ledger windows for `/kanban` Ctrl+D current/last API-cache stats, including Codex request-mode counts; it prefers canonical event boundaries and retains derived-SQLite fallback only for compatibility with old/incomplete detail-view directories |
| `SumMoltSessionTokenLedgerFromEvents` | `tui/internal/fs/agent.go:664-672`, `tui/internal/fs/molt_window.go:41-102` | process-free home-telemetry variant: incrementally resolves `psyche_molt` boundaries only from canonical `logs/events.jsonl`, degrades to empty session stats when that source is unavailable, and never invokes SQLite; the per-agent offset cache parses only appended ranges, retries partial EOF rows, resets on replacement/observed shrink, and uses a bounded completed-prefix anchor to detect same-inode truncate-and-regrow before carrying boundaries forward |
