...
| `DirectUnreadStore` / `OpenDirectUnreadStore` | `tui/internal/fs/direct_unread.go:17-23`, `tui/internal/fs/direct_unread.go:103-153` | stores project-local version-1 direct-thread cursors and performs open/read/baseline/save as one stable-path transaction using only caller-supplied accepted mail |
| `SyncTargets` / `UnreadCount` / `MarkSeen` | `tui/internal/fs/direct_unread.go:156-291` | adds but never prunes stable keys, keeps cached unread reads, and transactionally refreshes then copy-on-write saves monotonic cursor changes before publishing memory |
