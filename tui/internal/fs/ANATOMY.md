...
| `SumMoltSessionTokenLedger` | `tui/internal/fs/agent.go:643-658`, `tui/internal/fs/molt_window.go:38-82` | incrementally resolves `psyche_molt` boundaries in-process from canonical `logs/events.jsonl` (derived SQLite only when the canonical log is unavailable), then sums cached non-daemon token-ledger windows for home telemetry and `/kanban` Ctrl+D current/last API-cache stats, including Codex request-mode counts; the per-agent offset cache parses only appends and resets on replacement/truncation |
