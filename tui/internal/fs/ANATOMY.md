...
| `CapabilitiesForDisplay(manifest)` | `tui/internal/fs/agent.go:100` | prepends intrinsic caps (`system, email, psyche`) to manifest caps, deduped, for operator display (kanban/props) |
| `ReadInitManifest(dir)` | `tui/internal/fs/agent.go:129` | prefers `system/manifest.resolved.json`, falls back to `init.json`, and flattens `llm.*` (a legacy `soul` block is carried through but not flattened) |
| `WritePrompt` | `tui/internal/fs/agent.go:212` | writes `.prompt` signal file (TUI→agent injection) |
| `IsOrchestratorManifest(manifest)` | `tui/internal/fs/agent.go:232` | lower-level orchestrator role detector shared by TUI display logic and running-agent inventory |
| `DiscoverAgents(baseDir)` | `tui/internal/fs/agent.go:258` | scans for all subdirectories with `.agent.json` |
| `ReadStatus(dir)` | `tui/internal/fs/agent.go:400` | reads `.status.json` → `AgentStatus` with whole-file semantics retained for unrelated callers |
| **agent_record.go** | | |
...
| **direct_mail.go** | | |
| `DirectTarget` / `DirectThreadKey` / `AddressFingerprint` | `tui/internal/fs/direct_mail.go:10-40` | target carries canonical project + target directories (the target directory doubling as its route's absolute spelling), durable manifest AgentID, and current route; thread identity hashes `(project, agent_id)`, while the address fingerprint is route-only |
| `NormalizeMailEndpoints` / `IsDirectMail` | `tui/internal/fs/direct_mail.go:42-206` | keeps lenient deduplication for topology, but direct membership requires one valid raw recipient, empty CC, distinct endpoints, project-aware route equality (bare address or exact canonical absolute directory), and matching supplied inbound `identity.agent_id` |
| `directEndpoint` / `directSpelling` / `directTargetEndpoint` / `directHumanEndpoint` | `tui/internal/fs/direct_mail.go:114-164` | the one route-equality rule shared by `IsDirectMail` and the publication: a participant is named only by its trimmed bare address or its exact lexically cleaned absolute directory (the target's own `Directory`; `<ProjectDirectory>/.lingtai/<humanAddress>` for the human), so shared final components, other agents' directories, cross-project/foreign/nested/relative paths never match, and a blank or relative `Directory`/`ProjectDirectory` contributes no absolute spelling |
| **direct_publication.go** | | |
| `DirectMailPublication` / `NewDirectMailPublication` | `tui/internal/fs/direct_publication.go:16-95` | immutable, indexed accepted-direct snapshot: one pass over accepted mail selects same-strict-peer-route candidates (`directMailPeerAddress`, `tui/internal/fs/direct_publication.go:97-121`, which recognizes every accepted human spelling while each target is indexed under both its bare address and its absolute directory), keeps `IsDirectMail` as the final predicate, and retains per-thread detached chronological messages plus pre-resolved incoming unread summaries (accepted mail whose sender is not the human) and the latest monotonic boundary |
| `DirectPage` | `tui/internal/fs/direct_publication.go:160-178` | newest-horizon chronological page plus `hasOlder` for exactly one validated stable route; O(page) work/allocation independent of unrelated accepted mail |
| publication unread accessors | `tui/internal/fs/direct_publication.go:194-229` | fail-closed per-thread incoming summaries, latest boundary, and human-address/target-set validation consumed by the publication-aware unread store APIs |
| **direct_unread.go** | | |
...
- **Writes signal files** (the only agent-owned files the TUI writes): `.sleep`, `.suspend`, `.interrupt`, `.prompt`, `.refresh`/`.refresh.taken`. `.inquiry` is retired and is never written.
...
- **Direct mail identity boundary.** `DirectTarget` separates stable identity (canonical project directory + manifest `agent_id`) from current routing (target directory + address); `DirectThreadKey` hashes only the stable pair, and `AddressFingerprint` is route-only. `NormalizeMailEndpoints` remains deliberately lenient for topology edges. `IsDirectMail` instead validates one raw recipient, rejects any CC, malformed/multi-entry envelope, empty/equal endpoints, or cross-route record, and on incoming mail requires any supplied nonblank `identity.agent_id` to match literally while allowing route-equality fallback for legacy mail without that field. Route equality is project-aware because manifests and TUI sends use bare addresses while real kernel `email.reply` envelopes record canonical absolute agent directories: the human and the target are each named by their bare address or their exact absolute directory (the target's `Directory`; `<ProjectDirectory>/.lingtai/<humanAddress>` for the human) and by nothing else — never a shared final path component, a stripped prefix, another agent's or project's directory, or a relative path. Absolute spellings are compared lexically cleaned with no symlink resolution or I/O, so a symlink-divergent spelling fails closed instead of misrouting.
