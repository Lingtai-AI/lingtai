...
| **agent_record.go** | | |
| `ReadAgentRecord(dir)` / `AgentRecord.FreshAsyncWork(now)` | `tui/internal/fs/agent_record.go:109-296` | reads kernel-published `system/agent_record.json` (top schema `lingtai.agent_record/v1`) for Home telemetry and retains optional `async_work` raw so a malformed/missing child cannot poison valid top-level model/usage. `FreshAsyncWork` is the sole strict child decoder: exact `lingtai.async_work/v1`/version/window, required nonnegative integer aggregate + daemon/Shell lane counts, overflow-safe lane equality, required daemon usage, safe positive detail maps, and RFC3339 freshness age in inclusive `[0,600s]`; any mismatch returns unavailable for the async consumer only |
...
- **Reads from agent working directories** — `.agent.json`, `.agent.heartbeat`, `.status.json`, `system/agent_record.json`, `mailbox/*/`, token/event/history JSONL, topology/contact state, `daemons/.dispatch-ledger.jsonl`, and exact dispatched run cards/ledgers. Kanban uses only the bounded variants above.
