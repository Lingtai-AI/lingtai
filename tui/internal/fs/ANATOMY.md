...
| `SumMoltSessionTokenLedger` | `tui/internal/fs/agent.go:645-657` | sums cached non-daemon token-ledger windows for `/kanban` Ctrl+D current/last API-cache stats, including Codex request-mode counts; it prefers canonical event boundaries and retains derived-SQLite fallback only for compatibility with old/incomplete detail-view directories |
| `SumMoltSessionTokenLedgerFromEvents` | `tui/internal/fs/agent.go:664-671`, `tui/internal/fs/molt_window.go:22-160` | process-free home-telemetry variant: resolves the latest two valid `psyche_molt` boundaries only from canonical `logs/events.jsonl`, degrades to empty session stats when that source is unavailable, and never invokes SQLite; the stateless scanner snapshots one regular file, walks fixed 64 KiB chunks newest-to-oldest, buffers only a cross-chunk row, accepts a valid unterminated EOF row, skips malformed rows, and rejects concurrent mutation or path replacement. It reads arbitrary prior-prefix rewrites afresh and normally stops after the relevant two-session tail; histories with fewer than two boundaries scan to byte zero |
