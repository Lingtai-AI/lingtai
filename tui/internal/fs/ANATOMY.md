...
| `ReadStatus(dir)` | `tui/internal/fs/agent.go:355` | reads `.status.json` → `AgentStatus` (`tui/internal/fs/agent.go:306`): runtime plus the atomically-published token block — the current-session (since latest `psyche_molt`) scalars `input_tokens` / `output_tokens` / `thinking_tokens` / `cached_tokens` / `api_calls`, and the separate `context` sub-block. The five session scalars are the kernel's own since-molt counters (reset by `reset_session_token_usage` on a successful molt, preserved across a refresh), sharing the ledger-derived session boundary but not necessarily its totals because the ledger also counts `source=soul` rows; Home reads the published counters instead of re-deriving them (issue #643). The sibling `tokens.total_tokens` is deliberately not decoded (it duplicates input+output+thinking). Missing/unreadable/malformed status → zero struct; valid legacy status preserves fields it has while absent economy scalars decode to zero; a pre-current populated snapshot may have wider scope, never an error |
| `ReadContextStats(dir)` | `tui/internal/fs/agent.go:368` | summarizes retained `history/chat_history.jsonl`: entries, role counts, text input/output, tool calls/results, and per-tool distribution |
| `AggregateTokens(dirs)` | `tui/internal/fs/agent.go:497` | sums `TokenTotals` across multiple agent ledgers |
| `SumTokenLedger(path)` | `tui/internal/fs/agent.go:514` | sums a single main-agent `token_ledger.jsonl` → `TokenTotals`, skipping historical daemon-mirrored rows (`source=daemon`, `em_id`, or `run_id`) |
| `SumTokenLedgerByProvider` | `tui/internal/fs/agent.go:628` | groups main-agent ledger entries by derived provider name + recent N entries, skipping daemon-mirrored rows so `/kanban` main detail stays separate from daemon detail |
| `SumMoltSessionTokenLedger` | `tui/internal/fs/agent.go:668` | uses `logs/log.sqlite` `psyche_molt` boundaries when available (JSONL fallback), then sums cached non-daemon token-ledger windows for `/kanban` Ctrl+D current and last session API/cache stats, including Codex `codex_request_mode` counts (`ws_full` / `ws_incremental`) |
| `SumMoltSessionToolCalls` | `tui/internal/fs/agent.go:751` | counts lifecycle `tool_call` events in the SAME current/previous molt windows as `SumMoltSessionTokenLedger` (via `sqlitelog.QueryMoltSessionToolCallCounts`, JSONL fallback) for the `/kanban` Ctrl+D `tool_calls` + `tool_calls/api_call` rows; tool results are not counted. Freshness is keyed on authoritative `events.jsonl` (derived `log.sqlite` only when JSONL is absent), NOT the token-ledger cache, so event-only changes invalidate the count and SQLite fallback cannot pin a stale result |
| `SumSessionTokenLedgerBetween` | `tui/internal/fs/agent.go:912` | reusable `[since, before)` ledger-window summation helper used by molt-session stats and since-cutoff callers |
| **rebuild_marker.go** | | |
...
| `DeriveLedgerProvider` | `tui/internal/fs/agent.go:1016` | maps endpoint host / model prefix → canonical provider name |
| **heartbeat.go** | | |
