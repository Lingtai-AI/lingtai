...
| **session.go** | | |
| `SessionCache` | `tui/internal/fs/session.go:119` | append-only cache backed by `session.jsonl`; tails mail + events + inquiries |
| `NewSessionCache` | `tui/internal/fs/session.go:148` | creates cache, calls `RebuildFromSources` after construction |
| `RebuildFromSources` | `tui/internal/fs/session.go:164` | full ingest from mail cache + SQLite/JSONL event replay + soul_inquiry.jsonl + soul_flow.jsonl |
| `Refresh` | `tui/internal/fs/session.go:1347` | incremental poll of all three sources |
| **project_hash.go** | | |
