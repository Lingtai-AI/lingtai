...
| `RebuildFromSources` / `RebuildFromSourcesInMemory` | `tui/internal/fs/session.go` | authoritative full ingest; write-through for accepted callers or detached/no-persist for generation-gated Mail work |
| `RebuildFromSourcesWindowedInMemory` / `Complete` | `tui/internal/fs/session.go` | O(window) first-frame variant: ingests only the newest `window` session events (via `sqlitelog.StreamSessionEventsWindow`) plus all mail/inquiries, but still advances `eventsOff` to the true EOF boundary so `Refresh` resumes from EOF (no re-ingest of the excluded older window). When the window truncates older events the cache is left partial — `Complete()` returns false and `Persist` refuses to rewrite `session.jsonl` |
| `Persist` | `tui/internal/fs/session.go` | writes an accepted detached snapshot ONLY when `Complete()` (a partial windowed cache is never persisted as if complete), creating the derived cache parent directory at the write boundary |
...
- **Session cache reconstruction.** `RebuildFromSources` is idempotent — it re-ingests all mail + events + inquiries from offset 0, sorts by timestamp, and rewrites `session.jsonl`; `RebuildFromSourcesInMemory` performs the same read/merge without filesystem writes for detached generation-gated work. It prefers `logs/log.sqlite` only when coverage and rows prove the exact canonical root `logs/events.jsonl` source identity (`source_kind=agent_events`, `scope=agent`), otherwise it falls back to authoritative JSONL. Coverage and streaming share one captured `MaxOffset`; rows above that horizon remain JSONL work, and a non-positive indexed boundary rejects SQLite before any rows are appended (`tui/internal/sqlitelog/event.go`, `tui/internal/fs/session.go`). Every path retains the last complete-record boundary it actually consumed, so trailing partial records and concurrent appends are retried by `Refresh` rather than leaked, duplicated, or skipped.
- **Windowed first-frame reconstruction (newest-N).** `RebuildFromSourcesWindowedInMemory(cache, humanAddr, orchDir, orchName, window)` is the O(window) launch path: for a positive `window` it ingests only the newest `window` session events (indexed via `sqlitelog.StreamSessionEventsWindow`, which orders by `source_offset DESC LIMIT window` so the unique `idx_events_source_offset` index short-circuits after `window` rows instead of scanning the whole source), while still loading all mail and inquiries. Regardless of windowing, `eventsOff` is advanced to the true EOF complete-record boundary (`indexedJSONLBoundary`), so a later `Refresh` resumes from EOF and never re-ingests the excluded older window — no duplicates, no skips. A truncated window leaves the cache **partial** (`Complete()==false`), and `Persist` refuses to rewrite `session.jsonl` from a partial cache so the operator's complete derived file is never truncated. `window <= 0` is a full, complete rebuild. The **indexed** path is O(window); when no usable SQLite index exists the JSONL correctness fallback still reads the full file (complete, slower) — that slowness is confined to the non-indexed fallback, never the indexed first frame. Older history is fetched only by explicit upward navigation (the Mail Ctrl+U older-page loader re-runs this rebuild with a larger window, generation-gated). Two correctness reaches keep windowing from stranding history: (1) when the indexed rows do **not** fill a positive window (`seenRows < window`, i.e. the window is exhausted / the caller is loading older pages), the pre-index JSONL prefix `[0, MinOffset)` is stitched for the newest still-missing events, so a tail-only index can never permanently hide its prefix; a window that reaches offset 0 becomes `Complete()` and persistable. This prefix scan never runs on the ordinary fully-indexed first frame (there `seenRows == window`). (2) When a filled window's OLDEST loaded entry is a legacy api-grouped entry with no explicit `api_call_id` (`needsGroupBackExtension`), the ingest reaches back JUST to the nearest `llm_response` header via `sqlitelog.QueryWindowGroupBoundary` + `StreamSessionEventsOffsetRange` (an `llm_call` reset is left alone), so `buildMessages` derives the same group id the full-history render would and no spurious api-call-group separator appears at the window's top edge. Completeness is fresh-cache-compatible: `NewSessionCache` initializes `complete=true`, so a plain `NewSessionCache`+full `Refresh`+`Persist` (no windowed rebuild) still writes, exactly as before windowing; only a proven window truncation flips it false.
