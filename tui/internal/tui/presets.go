...
func (m PresetsModel) viewList() string {
	...
	// Title bar
	title := StyleTitle.Render(i18n.T("app.title")) + " " + StyleAccent.Render("·") + " " + StyleTitle.Render(i18n.T("presets.title"))
	escHint := lipgloss.NewStyle().Foreground(ColorAccent).Render("[esc] ") + StyleSubtle.Render(i18n.T("presets.back"))
...
func (m PresetsModel) viewNew() string {
	...
	title := StyleTitle.Render(i18n.T("app.title")) + " " + StyleAccent.Render("·") + " " + StyleTitle.Render(i18n.T("presets.new"))
	b.WriteString(title + "\n")
	b.WriteString(strings.Repeat("─", m.width) + "\n\n")
