...
- **`mail.go:103-1204`** (1204 lines) — `MailModel`. The network home screen — async message thread, compose input, slash-command palette, heartbeat pulse, agent state, and project-level activity badge. Constructor: `NewMailModel` (`mail.go:146`). Refresh/poll ticks, message history pagination, and the `/human` compose path that writes to `human/mailbox/outbox/`.
