...
// "Email To:" footer indicator gains a compact Claude-Code-style suffix beside
// `Active N s`:
//
//	◉ active 12s · 1024 tok downloaded
//
// where the token count is the documented integer `streamed_chars / 4`.
//
...
// streamProgressSuffix returns the localized ` · N tok downloaded` fragment
// for the footer indicator, or "" when nothing may be shown. It reads cached
// state only — no I/O — and re-checks the recipient identity so a stale
// reading can never render beside a different recipient.
func (m MailModel) streamProgressSuffix(state string) string {
	...
	return " · " + i18n.TF("mail.stream_progress_tok", m.streamProgress.snapshot.EstimatedTokens())
}
