...
| **Capabilities** | Composable capabilities (file [read/write/edit/glob/grep], psyche, bash, vibing, delegate, email, vision, web_search, talk, compose, draw, listen) | Declared at construction via `capabilities=` on Agent |
...
| `vibing` | `capabilities=["vibing"]` or `{"vibing": {"interval": 300}}` | Idle-breaking sticky note (hormê). Agent writes a disposable vibe via `vibe` action, toggles via `switch` action. Each nudge git-committed to `vibing/vibe.md`. Rewrite every time — ephemeral, not persistent. |
