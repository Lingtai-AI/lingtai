...
def test_listen_starts_all_accounts():
    ...
    with (
        patch.object(svc.accounts[0], "start_listening", create=True) as mock_listen_0,
        patch.object(svc.accounts[1], "start_listening", create=True) as mock_listen_1,
    ):
        svc.listen(on_message)
    mock_listen_0.assert_called_once_with(on_message)
    mock_listen_1.assert_called_once_with(on_message)


def test_stop_stops_all_accounts():
    svc = IMAPMailService(accounts=[ACCOUNT_1, ACCOUNT_2])
    with (
        patch.object(svc.accounts[0], "stop_listening", create=True) as mock_stop_0,
        patch.object(svc.accounts[0], "disconnect") as mock_disc_0,
        patch.object(svc.accounts[1], "stop_listening", create=True) as mock_stop_1,
