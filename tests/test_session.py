...
def test_send_stale_interaction_recovery():
    ...
    from stoai.llm_utils import _is_stale_interaction_error
    ...
    with patch("stoai.session.send_with_timeout", side_effect=fake_send_with_timeout), \
         patch("stoai.session._is_stale_interaction_error", return_value=True):
        response = sm.send("hello")
...
def test_send_non_stale_error_propagates():
    ...
    with patch("stoai.session.send_with_timeout", side_effect=ValueError("real error")), \
         patch("stoai.session._is_stale_interaction_error", return_value=False):
...
def test_send_preserves_interaction_id():
    ...
    with patch("stoai.session.send_with_timeout", return_value=mock_session.send.return_value):
        sm.send("hello")
...
def test_on_reset_summarizes_tool_calls():
    ...
    from stoai.llm.interface import ToolCallBlock
