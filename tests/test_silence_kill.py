...
def test_non_admin_cannot_send_silence_via_mail(tmp_path):
    ...
    agent = BaseAgent(
        service=make_mock_service(), agent_name="test", agent_id="test", base_dir=tmp_path, admin={},
    )
    mock_mail = MagicMock()
...
def test_admin_can_send_silence_via_mail(tmp_path):
    ...
    import time
    agent = BaseAgent(
        service=make_mock_service(), agent_name="test", agent_id="test", base_dir=tmp_path, admin={"silence": True, "kill": True},
    )
    mock_mail = MagicMock()
    mock_mail.address = "127.0.0.1:8000"
...
def test_admin_dict_stored(tmp_path):
    ...
    agent_normal = BaseAgent(service=make_mock_service(), agent_name="a", agent_id="t1", base_dir=tmp_path)
    assert agent_normal._admin == {}

    agent_admin = BaseAgent(service=make_mock_service(), agent_name="b", agent_id="t2", base_dir=tmp_path,
                            admin={"silence": True, "kill": True})
    assert agent_admin._admin == {"silence": True, "kill": True}

    agent_peer = BaseAgent(service=make_mock_service(), agent_name="c", agent_id="t3", base_dir=tmp_path,
                           admin={"silence": True})
