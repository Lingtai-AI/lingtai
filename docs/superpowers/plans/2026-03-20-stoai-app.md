# StoAI App Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** A single `stoai` CLI command that launches an autonomous orchestrator agent with IMAP, Telegram, and CLI channels.
...
**Tech Stack:** Python stdlib + stoai framework. No new dependencies.

**Spec:** `docs/superpowers/specs/2026-03-20-stoai-app-design.md`

**Key conventions:**
- `from __future__ import annotations` in every file
- All tests use `unittest.mock.MagicMock`
- Test functions: `test_<what_is_tested>`
- `app/` is NOT inside `src/stoai/` — it's a top-level application directory
...
3. **Add `stoai send` implementation and test.** In `app/__init__.py`, handle `sys.argv` with `send` subcommand. Implementation:
...
- `stoai` or `stoai config.json` → launch
- `stoai send "message"` → one-shot send via TCP
...
// config.example.json
{
  "_comment": "StoAI orchestrator config. Copy to config.json and fill in.",
...
Run: `source venv/bin/activate && pip install -e . && stoai --help 2>&1 || echo "entry point registered"`

- [ ] **Step 4: Commit**

```bash
git add pyproject.toml config.example.json model.example.json
git commit -m "feat(app): add stoai CLI entry point and example configs"
```
...
```bash
git add -A
git commit -m "feat(app): stoai orchestrator CLI — complete"
```
