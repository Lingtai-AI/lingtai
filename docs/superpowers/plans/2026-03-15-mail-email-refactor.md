...
|-----|-----|---------------|
| `src/stoai/services/email.py` | `src/stoai/services/mail.py` | `MailService` ABC + `TCPMailService` (was `EmailService`/`TCPEmailService`) |
| `src/stoai/intrinsics/email.py` | `src/stoai/intrinsics/mail.py` | `mail` intrinsic schema: FIFO send/check/read |
| `tests/test_services_email.py` | `tests/test_services_mail.py` | Tests for `TCPMailService` |

### Modified files
| File | What changes |
|------|-------------|
| `src/stoai/agent.py` | `email_service` → `mail_service`, `_email_service` → `_mail_service`, replace mailbox with FIFO `_mail_queue`, simplify `_on_mail_received` to just enqueue, strip all email methods down to FIFO send/check/read, remove reply/reply_all, rename intrinsic from `"email"` to `"mail"` |
| `src/stoai/intrinsics/__init__.py` | `email` → `mail` in ALL_INTRINSICS |
| `src/stoai/__init__.py` | `EmailService`/`TCPEmailService` → `MailService`/`TCPMailService` |
| `src/stoai/capabilities/__init__.py` | Add `"email"` capability, remove `"group_chat"` |
...
| File | Responsibility |
|------|---------------|
| `src/stoai/capabilities/email.py` | `email` capability: mailbox, check, read, reply, reply_all, cc, bcc, multi-to send. Intercepts mail receive to box messages. |
| `tests/test_layers_email.py` | Tests for the `email` capability |

### Deleted files
| File | Why |
|------|-----|
| `src/stoai/services/email.py` | Replaced by `services/mail.py` |
| `src/stoai/intrinsics/email.py` | Replaced by `intrinsics/mail.py` |
| `tests/test_services_email.py` | Replaced by `test_services_mail.py` |
| `src/stoai/capabilities/group_chat.py` | Merged into `capabilities/email.py` |
...
**Files:**
- Delete: `src/stoai/services/email.py`
- Create: `src/stoai/services/mail.py`
...
Copy `tests/test_services_email.py`, rename all `TCPEmailService` → `TCPMailService`, update import path to `stoai.services.mail`.
...
- [ ] **Step 4: Update src/stoai/__init__.py**
...
Delete `src/stoai/services/email.py` and `tests/test_services_email.py`.

- [ ] **Step 6: Smoke-test import**

Run: `source venv/bin/activate && python -c "from stoai import MailService, TCPMailService; print('ok')"`
Expected: `ok`

- [ ] **Step 7: Commit**

```bash
git add src/stoai/services/mail.py tests/test_services_mail.py src/stoai/__init__.py
...
**Files:**
- Delete: `src/stoai/intrinsics/email.py`
- Create: `src/stoai/intrinsics/mail.py`
...
Run: `source venv/bin/activate && python -c "from stoai.intrinsics import ALL_INTRINSICS; assert 'mail' in ALL_INTRINSICS; assert 'email' not in ALL_INTRINSICS; print('ok')"`
Expected: `ok`

- [ ] **Step 5: Commit**

```bash
git add src/stoai/intrinsics/mail.py src/stoai/intrinsics/__init__.py
git add -u
...
Run: `source venv/bin/activate && python -c "import stoai" && python -m pytest tests/test_agent.py tests/test_services_mail.py -v`
Expected: All PASS
...
**Files:**
- Create: `src/stoai/capabilities/email.py`
- Delete: `src/stoai/capabilities/group_chat.py`
...
Delete `src/stoai/capabilities/group_chat.py` and `tests/test_layers_group_chat.py`.
...
```bash
git add src/stoai/capabilities/email.py src/stoai/capabilities/__init__.py tests/test_layers_email.py
git add -u
...
- Modify: `src/stoai/__init__.py`
- Modify: `src/stoai/capabilities/delegate.py` (comment only)

- [ ] **Step 1: Update examples/two_agents.py**

- `from stoai.services.email import TCPEmailService` → `from stoai.services.mail import TCPMailService`
- `TCPEmailService(...)` → `TCPMailService(...)`
...
- [ ] **Step 4: Update src/stoai/__init__.py exports**
...
```bash
git add examples/ src/stoai/__init__.py src/stoai/capabilities/delegate.py
...
```bash
python -c "
from stoai import MailService, TCPMailService, EmailManager
from stoai.services.mail import MailService, TCPMailService
from stoai.capabilities.email import EmailManager
