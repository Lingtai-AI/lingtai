...
- **`.portal/topology.jsonl`** — JSONL tape of network snapshots. Each line is `{"t": <unix_ms>, "net": <Network>}`. Appended every 3 seconds by `StartRecording` (`portal/internal/api/server.go:68-112`) using the fast no-mail live graph path; explicit replay reconstruction may replace it with a reconstructed tape.
...
- **`.portal/reconstruct.progress`** — Temporary `"N/M"` progress file during explicit tape reconstruction. Normal startup does not create it; the shared replay writer updates it while caching reconstructed frames (`portal/internal/api/replay.go:417-446`).
...
- **Live recording begins at startup.** `StartRecording` (`portal/internal/api/server.go:68-112`) runs in a background goroutine and records a fast no-mail live snapshot immediately, then every 3 seconds. It deliberately does not reconstruct historical replay on open; `POST /api/topology/rebuild` is the explicit full-rebuild path.
