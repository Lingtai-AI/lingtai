...
- **`install.sh`** — One-command source installer (`curl … | bash`). Builds both binaries from source, installs them into the selected bin directory (Homebrew prefix, writable `/usr/local/bin`, or `~/.local/bin`), stamps exact `vX.Y.Z` release-tag installs as that tag, and writes source install metadata to `~/.lingtai-tui/install.json`. Auto-detects CN-restricted networks and falls back to mirrors for Go modules / `npm` / Go checksum DB.
