...
- **`install.ps1`** — Experimental native Windows installer (`iwr -useb https://lingtai.ai/install.ps1 | iex`). PowerShell 5.1+, Windows-only (fails clearly elsewhere). Downloads the prebuilt Windows zip (`lingtai-<tag>-windows-<arch>.zip`, produced by `.github/workflows/release.yml`); does NOT build from source. Installs `.exe` binaries into a per-user bin dir (`%LOCALAPPDATA%\Programs\lingtai\bin`), adds it to the user PATH, and provisions the Python runtime venv with `uv` at `~/.lingtai-tui/runtime/venv`. Writes **diagnostic-only** `install.json` (`install_method: "powershell"`, `install_kind: "powershell-prebuilt"`) — deliberately NOT `source`, so the TUI source updater does not try to bash-update it. There is no native PowerShell self-update: re-run the one-liner to upgrade. Native Windows is reduced-capability (daemon/subagents unavailable, `bash` tool runs cmd.exe); WSL2 + `install.sh` remains the full-parity path. Smoke-tested by `scripts/test-install-ps1.ps1`.
- **`scripts/`** — Auxiliary Python utilities (image-to-blocks, Telegram chat dumper, tool description dumper, file-rename helper) plus `test-install-ps1.ps1`, the parse + `-DryRun` smoke test for `install.ps1` run by `.github/workflows/windows-installer-smoke.yml`. NOT the runtime — these are dev-only tools.
...
- **`.github/workflows/release.yml`** — Tag-push release workflow. On `v*` tags the `release-assets` job cross-builds `lingtai-tui` + `lingtai-portal` for darwin/linux/windows × amd64/arm64: darwin/linux ship as `lingtai-<tag>-<os>-<arch>.tar.gz`, windows (cross-compiled on a Linux runner, CGO off) ships as `lingtai-<tag>-windows-<arch>.zip` with `.exe` binaries, each with a `.sha256`. `install.sh` consumes the tarballs; `install.ps1` consumes the zips. The `update-homebrew` job computes the source tarball checksum, rewrites `lingtai-tui.rb`, and pushes the formula update to `Lingtai-AI/homebrew-lingtai`.
- **`.github/workflows/windows-installer-smoke.yml`** — PR/`workflow_dispatch` validation on `windows-latest`, scoped via `paths` to `install.ps1` / `scripts/test-install-ps1.ps1` / itself. Runs `pwsh -NoProfile -File scripts/test-install-ps1.ps1` (parse + AST + `-DryRun -SkipVenv`), with no network or install side effects.
